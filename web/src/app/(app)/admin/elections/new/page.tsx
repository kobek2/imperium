...
export default async function NewElectionPage() {
  ...
  const [wards, totalPlayers] = supabase
    ? await Promise.all([loadWardPopulations(supabase), countTotalPlayers(supabase)])
    : [[], 0];
    ...
    <div className="mx-auto w-full max-w-2xl space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--psc-muted)]">
          NYC city races only (mayor + council districts). Phases are advanced manually from the election detail page.
        </p>
      </div>
      <CreateElectionForm action={createElection} wards={wards} totalPlayers={totalPlayers} />
    </div>
  );
}
