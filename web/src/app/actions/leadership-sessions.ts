 ...
 *
 * Falls back to democrat if the chamber is empty (no one to participate anyway).
 */
async function inferMajorityParty(
  ...
  const { data: profiles } = await supabase
    ...
    .select("id, party")
    .in("id", ids);
  const counts = { democrat: 0, republican: 0, independent: 0 } as Record<PartyKey, number>;
  for (const p of profiles ?? []) {
    const party = assertParty(p.party);
    if (party) counts[party]++;
  }
  const winner = (Object.entries(counts) as [PartyKey, number][]).sort(
    (a, b) => b[1] - a[1],
  )[0]?.[0];
  return winner ?? "democrat";
}
