...
export function OpenSeatFilingForm({
  electionId,
  jurisdictionPlayerCount,
}: {
  electionId: string;
  /** Profiles whose character matches this seat’s jurisdiction (house district / senate state / any for president). */
  jurisdictionPlayerCount?: number;
}) {
  return (
    <div className="space-y-2">
      {typeof jurisdictionPlayerCount === "number" ? (
        <p className="text-[11px] font-semibold text-amber-950">
          Players in this jurisdiction:{" "}
          <span className="font-mono">{jurisdictionPlayerCount}</span>
          {jurisdictionPlayerCount < 1 ? (
            <span className="ml-1 font-normal text-amber-900/90">
              — filing will still open if you confirm (e.g. NPC-only seat).
            </span>
          ) : null}
        </p>
      ) : null}
