-- Raise public company founding fee from $10M to $100M.
...
declare
  v_uid uuid := auth.uid();
  nominal_founding_fee numeric := 100000000;
...
begin
  ...
  if wallet_charge > 0 and w.balance < wallet_charge then
    raise exception 'Insufficient balance — $100,000,000 founding fee required';
  end if;
