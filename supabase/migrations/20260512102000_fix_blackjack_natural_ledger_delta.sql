-- Natural blackjack already pays the wallet correctly at 3:2 after the ante is
-- deducted. The ledger payout delta must record the full returned amount
-- (stake + winnings = 2.5x), not just the net profit (1.5x).
...
begin
  ...
  if p_nat and not d_nat then
    ...
    update public.economy_wallets set balance = new_bal, updated_at = now() where user_id = v_uid;
    insert into public.economy_ledger (wallet_user_id, delta, balance_after, kind, detail)
    values (v_uid, bet * 2.5, new_bal, 'gamble_blackjack', jsonb_build_object('outcome', 'blackjack', 'player_hand', ph, 'dealer_hand', dh));
    return jsonb_build_object(
      'active', false,
      'outcome', 'blackjack',
