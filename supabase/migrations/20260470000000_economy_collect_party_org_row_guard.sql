-- economy_collect_income (current body from 20260464000000) still did
--   select * into po from party_organizations ...
-- with no row → "record po is not assigned yet" on every D/R hourly collect.
-- Treasury-only fix in 20260469000000 did not replace this function.
...
begin
  ...
  if v_party in ('democrat', 'republican') then
    ...
    if not found then
      raise exception 'Party treasury row missing for party %.', v_party;
    end if;
