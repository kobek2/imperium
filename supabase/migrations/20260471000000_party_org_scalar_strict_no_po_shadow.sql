-- "record po is not assigned yet" persists when:
--   (1) SELECT * INTO po returns no rows (composite record left uninitialized), or
--   (2) PL/pgSQL resolves `po` ambiguously vs subquery alias `FROM party_officers po`.
-- Fix: no composite `po`; lock row via scalar SELECT ... INTO STRICT after upsert.
-- Rename officer subquery alias to `po_ofc`.
...
begin
  ...
  begin
    select treasury_balance
      into strict v_treasury
    from public.party_organizations
    where party_key = p_party
    for update;
  exception
    when no_data_found then
      raise exception 'Party treasury row missing for party % (after upsert).', p_party;
  end;
...
begin
  ...
  if not exists (
    select 1 from public.party_officers po_ofc
    where po_ofc.party_key = p_party
      and po_ofc.office in ('chair', 'treasurer')
      and po_ofc.user_id = v_uid
  ) then
  ...
  if v_treasury < a then raise exception 'Insufficient party treasury balance'; end if;
