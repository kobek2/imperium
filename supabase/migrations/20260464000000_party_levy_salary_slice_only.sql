-- Party chair levy: only the government salary portion of hourly collect (role pay × hours), not PAC.
-- Withheld automatically when the member collects — no separate party payment step.

comment on column public.party_organizations.member_collect_levy_rate is
  'Party chair only: fraction of each member scheduled government salary in an hourly collect (role hourly × hours worked, excluding PAC hourly) sent to the party treasury when they collect. Members do not pay party tax separately.';
