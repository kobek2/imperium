-- Tax accounts are created in fiscal_close_year() on the row that is then marked `closed`.
-- Treasury RPCs previously joined only `status = 'active'`, so they never saw those rows → 0 warnings.
  ...
  for row_rec in
    ...
    where a.fiscal_year_id = y.id
      ...
      and (
        ...
        or (
          p_scope = 'due_soon'
          and a.due_at <= now() + make_interval(days => y.tax_warning_lead_days)
        )
        or (
          p_scope = 'delinquent'
          and (
