-- Fix RP-year tax scaling to match the federal budget bracket impact table:
-- the UI applies marginal brackets to **scheduled hourly gross** (treating that dollar amount as the bracket base),
-- then multiplies band tax by 72 (`TaxBracketAnalyticsTable`: revenueFromBand × m, totalTax × m).
-- That is **72 × fiscal_marginal_tax(hourly)**, not **fiscal_marginal_tax(hourly × 72)** (progressive brackets differ).
...
comment on function public.admin_fy4_apply_budget_tax_baseline() is
  'Staff (active FY4): sync enacted budget JSON, zero treasury + FY4 outlays + tax events, set paid=0. Assessed tax per profile = 72 × fiscal_marginal_tax(scheduled hourly role+PAC gross), matching federal `TaxBracketAnalyticsTable` / `estimatedBracketTaxRpYear` (not fiscal_marginal_tax(hourly×72)).';
...
comment on function public.fiscal_estimate_ytd_income_tax() is
  'RP-year federal income tax preview: 72 × fiscal_marginal_tax(scheduled hourly role+PAC gross), same scaling as the federal workbook bracket impact table (not marginal_tax on hourly×72).';
