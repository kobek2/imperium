...
begin
  ...
  select y.started_at into fy_started
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'user_id', p.id,
        'character_name', coalesce(nullif(trim(p.character_name), ''), p.discord_username, p.id::text),
        'withheld_total', round(coalesce(lev.total, 0), 2),
        'salary_base', round(coalesce(lev.salary_base, 0), 2),
        'received_total', round(coalesce(recv.total, 0), 2),
        'donated_total', round(coalesce(don.total, 0), 2)
      )
      order by greatest(coalesce(lev.total, 0), coalesce(recv.total, 0)) desc, p.character_name asc
    ),
    '[]'::jsonb
  )
  into levy_member_flows
  ...
  left join (
    select el.wallet_user_id as uid, sum(el.delta)::numeric as total
    from public.economy_ledger el
    where el.kind = 'party_treasury_in'
      and el.detail->>'party' = p_party
    group by el.wallet_user_id
  ...
  return jsonb_build_object(
    ...
    'salary_levy_member_flows', coalesce(levy_member_flows, '[]'::jsonb)
  );
end;
$$;
